...
export function getSamlOnboardingGuide(tenantId: string, preset: IdpPreset = 'generic_saml') {
  const base = process.env.PUBLIC_GATEWAY_URL || process.env.GATEWAY_URL || 'https://call-iq-gateway.onrender.com';
  ...
  return {
    ...
    steps: [
      ...
      `Set Entity ID: call-iq-${tenantId}`,
      'Upload IdP certificate to Call IQ SSO settings',
...
export async function getSamlMetadata(tenantId: string) {
  ...
  const base = process.env.PUBLIC_GATEWAY_URL || 'https://call-iq-gateway.onrender.com';
  return {
    tenantId,
    entityId: config?.entityId || `call-iq-${tenantId}`,
